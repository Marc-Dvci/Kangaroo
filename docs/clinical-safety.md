...
| **The cry analysis hears a phone microphone, not a clinician's ear.** | It emits `Auditory` provenance with a confidence below 1.0, may only escalate, and refuses any clip it cannot read. A recording containing no crying is refused rather than reported as an absent cry — the two are acoustically identical and clinically opposite, so the danger sign is recorded only when a person who tried to rouse the baby ticks it. |
